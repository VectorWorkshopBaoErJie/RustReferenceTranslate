...
* `Box<T>` 的 [解引用运算符][dereference operator] 产生一个可以被移动的地址。
  表示 `*` 运算符及 `Box<T>` 的析构函数是内置到语言中的。
...
它确保了编译器对此类类型进行正确的优化，还确保带有内部可变性的类型的 [`static` 条目][`static` items] 不会被放置在标记为只读的内存中。
{==+==}
...
[`std::marker::PhantomData<T>`] 是一种零大小、最小对齐的类型。
从 [协变性][variance]、[丢弃检查][drop check] 和 [自动 trait][auto traits](#auto-traits) 的角度来看，被视为拥有一个类型为 `T` 的实例。
...
[`Copy`] trait 改变了实现它的类型的语义。实现 `Copy` 的类型的值在赋值时会被复制而不是移动。
只有不实现 `Drop` trait 并且其所有字段都是 `Copy` 类型的类型才能实现 `Copy` trait 。
对于枚举，意味着所有变体的所有字段都必须是 `Copy` 类型。
对于联合体，意味着所有变体都必须是 `Copy` 类型。
...
[`Clone`] trait 是 `Copy` 的父级trait ，因此它也需要编译器生成的实现。编译器为以下类型实现了此 trait：
...
目前没有稳定的方法指定额外的否定实现；它们只存在于标准库中。自动 trait 可以作为 [trait 对象][trait object] 的附加约束添加到任何 trait 中，尽管通常只允许一个 trait 。
