...
* `Box<T>` 的 [解引用运算符][dereference operator] 产生可被移动的地址。
  `*` 运算符及 `Box<T>` 的析构函数是内置在语言中的。
* [方法][Methods] 能以 `Box<Self>` 作为接收者。
* 与相同 crate 中可以为 `T` 的 `Box<T>` 实现 trait ，而 [孤儿规则][orphan rules] 阻止了对其他泛型类型的实现。
{==+==}
...
[`Rc<Self>`] 可作为[方法][Methods]的接收者。

## `Arc<T>`

[`Arc<Self>`] 可作为[方法][Methods]的接收者。

## `Pin<P>`

[`Pin<P>`] 可作为[方法][Methods]的接收者。

## `UnsafeCell<T>`

[`std::cell::UnsafeCell<T>`] 用于 [内部可变性][interior mutability] 。
它确保了编译器对此类类型进行正确的优化，
还确保带有内部可变性的类型的 [`static` 条目][`static` items] 不会被放置在标记为只读的内存中。
{==+==}
...
[`std::marker::PhantomData<T>`] 是零大小、最小对齐的类型。
从 [协变性][variance] 、 [丢弃检查][drop check] 和 [自动 trait][auto traits](#auto-traits) 的角度来看，
被视为拥有类型为 `T` 的实例。
...
实现 `Copy` 的类型的值在赋值时会被复制而不是移动。
只有不实现 `Drop` trait 并且其所有字段都是 `Copy` 类型的类型才能实现 `Copy` trait 。
对于枚举，所有变体的所有字段都必须是 `Copy` 类型。
对于联合体，所有变体都必须是 `Copy` 类型。
编译器为以下类型实现了 `Copy` trait :
...
[`Clone`] trait 是 `Copy` 的父级 trait ，因此它也需要编译器生成的实现。
编译器为以下类型实现了此 trait：
...
所有用于不可变 [`static` 条目][`static` items] 的类型都必须实现此 trait 。

## `Termination`

[`Termination`] trait 指示 [main 函数][main function] 和 [test 函数][test functions] 可接受的返回类型。
{==+==}
...
[`Send`]、[`Sync`]、[`Unpin`]、[`UnwindSafe`] 和 [`RefUnwindSafe`] trait 是 "自动 trait" 。
...
对于泛型类型 (将上面内置类型视为泛型的 `T` )，如果有泛型实现，则编译器不会自动实现该 trait ，而是根据需要的 trait 约束为没有达到 trait 约束的类型实现。例如，标准库为所有 `T` 是 `Sync` 的 `&T` 实现了 `Send` ；这意味着，如果 `T` 是 `Send` 但不是 `Sync` ，则编译器不会为 `&T` 实现 `Send` 。

自动 trait 也可以有否定实现，在标准库文档中表示为 `impl !AutoTrait for T` ，它们覆盖了自动实现。例如， `*mut T` 有一个 `Send` 的否定实现，因此即使 `T` 是 `Send` ， `*mut T` 也不是 `Send` 。

目前没有稳定的方法指定额外的否定实现；它们只存在于标准库中。
自动 trait 可以作为 [trait 对象][trait object] 的附加约束添加到任何 trait 中，尽管通常只允许一个 trait 。
...
[`Sized`] trait 表示这个类型在编译时大小是已知的，即不是 [动态大小类型][dynamically sized type] 。
类型参数 (除了 trait 中的 `Self` ) 默认都是 `Sized` 的，关联类型也是。
