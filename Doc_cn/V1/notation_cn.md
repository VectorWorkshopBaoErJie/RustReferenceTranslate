...
译注：中文翻译无法表达驼峰形式，仅用斜体表示。无法表达大小写形式。如有必要可查看翻译仓库中的条目或原文。
{==+==}
...
entries.
{==+==}
在语法段中，一些规则，尤其是 [一元运算符][unary operators] 、 [二元运算符][binary operators] 和 [关键字][keywords] ，采用简化表示，是易阅读的字符串表示。
从而构成相关 [token][tokens] 规则的子集。假定在词法分析阶段由确定性有限自动机<abbr title="Deterministic Finite Automaton">DFA</abbr>驱动，操作生成的语法，产生对应字符串词条的分离分支。
{==+==}
