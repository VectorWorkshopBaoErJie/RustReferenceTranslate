...
> 注意: 由于 `repr(C)` 结构体和联合体的表示形式，如果一个变体只有一个字段，在联合体中直接放置该字段或将其包装在结构体中没有区别；
...
对于带字段的枚举，还可以将 `repr(C)` 和原始表示 (例如 `repr(C, u8)`) 结合使用。这会修改 [`repr(C)`] ，将判别值枚举的表示形式更改为所选择的原始表示。
因此，如果你选择了 `u8` 表示形式，则判别值枚举的大小和对齐方式将为 1 字节。来自 [之前][`repr(C)`] 示例的判别值枚举如下：
{==+==}
