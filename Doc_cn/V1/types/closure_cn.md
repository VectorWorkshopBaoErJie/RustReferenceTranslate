...
closure:
{==+==}
[闭包表达式][closure expression] 生成一个闭包值，具有独特的匿名名称的类型，无法显式书写。
...
编译器优先选择对闭包变量进行不可变借用，然后是唯一不可变借用 (参见下文) ，然后是可变借用，最后才是移动的方式。
编译器将选择与在闭包体内使用捕获变量的方式兼容的这些选项中的第一个。编译器不考虑周围的代码，比如涉及变量的生命周期或闭包本身的生命周期。

如果使用了 `move` 关键字，则所有的捕获都将移动，或者对于 `Copy` 类型将复制，而不分析是否可以借用。
通常， `move` 关键字应用于允许闭包超出捕获值的生命周期，例如，如果闭包正在被返回或用于生成新线程。

结构体、元组和枚举等复合类型始终被整体捕获，而不是逐个字段捕获。要捕获单个字段，可能需要先借用到一个局部变量中：
{==+==}
...
捕获可以通过一种称为唯一不可变借用的特殊借用方式，它无法在语言的其他地方使用，并且无法显式书写。
...
但与此同时，借用 `x` 为不可变引用将使赋值操作非法，因为 `& &mut` 引用可能不是唯一的，因而不能安全地用于修改值。
...
> 这是因为闭包类型实现的 trait 是由闭包对捕获值的操作方式决定的，而不是它们如何捕获其变量。

*非捕获闭包* 是不从其环境捕获任何内容的闭包，其可以强制转换为与签名匹配的函数指针 (例如, `fn()` ) 。
{==+==}
...
所有闭包类型都实现了 [`Sized`] 。此外，如果允许所存储的捕获类型这样做，则闭包类型会实现以下 trait：
{==+==}
