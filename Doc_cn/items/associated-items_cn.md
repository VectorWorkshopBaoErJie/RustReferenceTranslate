...
{==+==}
*关联条目* 是在 [traits] 中声明的条目或在 [implementations] 中定义的条目。
它们之所以被称为 "关联条目" ，是因为它们是在关联类型上定义的，即实现中的类型。
它们是您可以在模块中声明的条目种类的子集。具体而言，有 [关联函数][associated functions] (包括方法) 、 [关联类型][associated types] 和 [关联常量][associated constants] 。
{==+==}
...
{==+==}
一个常见的关联函数的例子是 `new` 函数，它返回与关联函数相关联类型的值。
{==+==}
...
{==+==}
当关联函数声明在 trait 上时，还可以使用一个 [路径][path] 来调用函数，该路径由 trait 名称附加而成。
当发生这种情况时，它将替换为 `<_ as Trait>::function_name` 。
{==+==}
...
{==+==}
第一个参数名为 `self` 的关联函数被称为 *方法* ，可以使用 [方法调用运算符][method call operator] 来调用它们，例如 `x.foo()` ，以及常规的函数调用表示法。
{==+==}
...
lifetime):
{==+==}
如果指定了 `self` 参数的类型，则仅限于由以下语法生成的类型 (其中 `'lt` 表示某个任意生命周期):
{==+==}
...
{==+==}
此语法中的 `Self` 表示一个解析为实现类型的类型。
这也可以包括上下文类型别名 `Self` 、其他类型别名或者解析为实现类型的关联类型投影。
{==+==}
...
{==+==}
如果 `self` 参数前缀带有 `mut` ，它将成为一个可变变量，类似于使用 `mut` [标识符模式][identifier pattern] 的常规参数。例如:
{==+==}
...
{==+==}
这定义了一个具有两种方法的 trait 。在 trait 处于作用域内时，所有具有 [实现][implementations] 的值都可以调用其 `draw` 和 `bounding_box` 方法。
{==+==}
...
{==+==}
> **版次差异**: 在2015版中，可以使用匿名参数 (例如 `fn foo(u8)` )声明 trait 方法。自2018版起，此操作已弃用并被视为错误。所有参数必须具有参数名称。
{==+==}
...
{==+==}
*关联类型* 是与另一个类型相关联的 [类型别名][type aliases] 。关联类型不能在 [内部实现][inherent implementations] 中定义，也不能在trait中提供默认实现。
{==+==}
...
{==+==}
*关联类型声明* 声明了关联类型定义的签名。
它使用以下形式之一书写，其中 `Assoc` 是关联类型的名称， `Params` 是逗号分隔的类型、生命周期或常量参数列表，
 `Bounds` 是加号分隔的 trait 约束列表 (关联类型必须满足这些约束），而 `WhereBounds` 是逗号分隔的参数必须满足的约束列表。
{==+==}
...
{==+==}
标识符是声明的类型别名的名称。可选的 trait 约束必须由类型别名的实现所满足。
关联类型上有一个隐式 [`Sized`] 约束，可以使用特殊的 `?Sized` 约束来放宽。
{==+==}
...
{==+==}
*关联类型定义* 为 trait 在类型上的实现定义了一个类型别名。它们的书写方式与 *关联类型声明* 相似，但不能包含 `Bounds` ，而必须包含 `Type` 。
{==+==}
...
{==+==}
如果类型 `Item` 从 trait `Trait` 中具有关联类型 `Assoc` ，那么 `<Item as Trait>::Assoc` 就是一个指定在关联类型定义中的类型的别名。
此外，如果 `Item` 是一个类型参数，那么可以在类型参数中使用 `Item::Assoc` 。
{==+==}
...
{==+==}
考虑以下 `Container` trait的示例。请注意，此类型可用于方法签名中。
{==+==}
...
{==+==}
为了使类型实现此trait，它不仅必须为每个方法提供实现，还必须指定类型 `E` 。这里是标准库类型 `Vec` 的 `Container` 的实现:
{==+==}
...
{==+==}
trait 上的泛型关联类型声明当前可能需要一系列 where子句 ，这取决于trait中的函数以及如何使用 GAT 。
未来这些规则可能会放宽；更新内容可以在 [泛型关联类型提案库](https://rust-lang.github.io/generic-associated-types-initiative/explainer/required_bounds.html) 中找到。
{==+==}
...
{==+==}
简而言之，为了最大限度地允许 impl 中的关联类型的定义，这些 where 子句是必需的。
为此，任何可以证明在函数上*保持* (使用函数或trait的参数) 的子句，其中 GAT 作为输入或输出出现时，必须在 GAT 本身上也写出来。
{==+==}
...
{==+==}
在上面的 `next` 函数中，我们可以证明 `Self:'a` ，因为由 `&'a mut self` 隐含的约束；因此，我们必须在 GAT 本身上写出等效约束： `where Self: 'x` 。
{==+==}
...
the union.
{==+==}
当 trait 中有多个使用 GAT 的函数时，则使用不同函数的约束的 *交集* ，而不是并集。
{==+==}
...
on `Checker`.
{==+==}
在这个例子中，对 `type Checker<'a>;` 不需要任何约束。虽然我们知道 `T:'a` 在 `create_checker` 上，但我们不知道在 `do_check` 上。
然而，如果注释掉了 `do_check` ，则需要在 `Checker` 上写出 `where T: 'x` 的约束。
{==+==}
...
the required bounds.
{==+==}
最后， trait 中对 GAT 的任何显式使用 `'static` 都不计入所需的约束。
{==+==}
...
{==+==}
只有在引用时，关联常量定义才会经历 [常量评估][constant evaluation] 。
此外，包含 [泛型参数][generic parameters] 的定义将在单态化之后进行评估。
{==+==}
